...
import { bookings, addBooking, findEventType, isSlotTaken, getBookings } from '../store';
...
router.post('/', (req, res) => {
  ...
  if (!isWithinInterval(bookingDate, { start: windowStart, end: windowEnd })) {
    return res.status(400).json({ code: 'booking_window_exceeded', message: 'Booking date is outside the allowed window' });
  }

  if (isSlotTaken(startAt)) {
    return res.status(409).json({ code: 'slot_unavailable', message: 'Slot already booked' });
  }

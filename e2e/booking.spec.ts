...
test("Гость может забронировать слот", async ({ page }) => {
  ...
  // Ждём, пока таблица обновится и появится строка с нашим заголовком
  // Используем title вместо id, потому что id может быть не виден, а title точно есть
  await expect(
    page.locator(`table tbody tr:has-text("${eventTitle}")`),
  ...
  // Ищем строку с нашим заголовком (eventTitle) и проверяем наличие имени гостя
  const row = page.locator(`table tbody tr:has-text("${eventTitle}")`);
...
test("Повторное бронирование того же слота возвращает ошибку", async ({
  ...
  // Ищем сообщение об ошибке (может быть в Alert, в Dialog или просто как текст)
  // Ищем по любому тексту, содержащему "already booked"
